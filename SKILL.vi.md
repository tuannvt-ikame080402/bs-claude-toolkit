...
Trước khi render brief, kiểm tra project health:
1. Tìm sprint plan mới nhất trong từng submodule trong SCOPE (`docs/plan/sprint-*.md`)
2. Đọc nó — quét DoD checkboxes (`- [x]` vs `- [ ]`)
3. Kiểm tra changelog / test doc / testlog đã tồn tại cho sprint slug đó chưa
...
──────────────────────────────────────────────────────────────
  ...
  Sprint đang mở: sprint-[N-1]-[slug]  ([ngày])
  DoD: [đã xong/tổng] item
    [ ] [item chưa xong 1]
    [ ] [item chưa xong 2]
  ...
  → Việc cần làm: [một hành động cụ thể — ví dụ "Chạy /review be" hoặc "Codex: tạo changelog"]
...
**Bước 1.5 — Impact analysis**

Trước khi viết plan, xác định những gì sẽ bị ảnh hưởng:

1. Từ research output, ghi nhận các file/function/class/endpoint mục tiêu.
2. Grep tìm nơi sử dụng các symbol đó trong các file *khác*:
   ```bash
   grep -r "[symbol]" {submodule} --include="*.py" -l   # điều chỉnh extension theo lang
   ```
3. Lập danh sách **AFFECTED** = các file gọi vào hoặc phụ thuộc vào vùng mục tiêu.
4. Với mỗi file bị ảnh hưởng, ghi chú ngắn: lý do bị ảnh hưởng và thay đổi có làm hỏng không.
...
**Bước 6 — Kiểm tra regression**

Lập **DEPENDENT_FILES**: các file *không* nằm trong CHANGED_FILES nhưng import hoặc gọi vào file đã thay đổi.

```bash
grep -r "[tên_module_thay_đổi]" {submodule} --include="*.py" -l   # điều chỉnh theo lang
```

Với mỗi function/method/class thay đổi hiển thị trong diff:
- Lấy signature cũ (từ dòng `-`) và signature mới (từ dòng `+`)
- Tìm call site dùng signature cũ trong DEPENDENT_FILES
- Với mỗi call site: đánh giá có tương thích với signature mới không

| Phát hiện | Hành động |
|-----------|-----------|
| Call site tương thích | ✓ không regression |
| Call site truyền arg đã xóa/đổi tên | ✗ regression — flag file:line |
| Call site expect return shape cũ | ✗ regression — flag file:line |
| Call site không tìm thấy (code không dùng) | ⚠ ghi chú thôi |
...
**changelog** — phải có:
- [ ] Header ngày hoặc ngày trong tên file
- [ ] Ít nhất một bullet point mô tả thay đổi thực sự (không phải template)
- [ ] Không còn placeholder (`[mô tả]`, `TODO`, v.v.)

**test doc** — phải có:
- [ ] Ít nhất một test case với input + expected output
- [ ] Có happy / edge / failure case
- [ ] Không còn placeholder

**testlog** — phải có:
- [ ] Kết quả pass/fail thực tế (không phải template)
- [ ] Tổng số passed / failed

| File | Trạng thái | Chất lượng |
|------|-----------|-----------|
| changelog | ✓ / ✗ | ✓ đầy đủ / ⚠ stub |
...
──────────────────────────────────────────────────────────────
  ...
  changelog  [✓ đầy đủ | ⚠ stub | ✗ thiếu]
  test doc   [✓ đầy đủ | ⚠ stub | ✗ thiếu]
  testlog    [✓ đầy đủ | ⚠ stub | ✗ thiếu]

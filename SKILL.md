...
### 1d. Extract stack profile

Từ CLAUDE.md / Agents.md đã load, extract và ghi nhớ:

| Biến | Tìm ở đâu | Ví dụ |
|------|-----------|-------|
| `lang_be` | Mục "Tech Stack" hoặc "Coding Conventions" | `Python`, `Go`, `TypeScript/Node`, `Java`, `PHP`, `Ruby` |
| `lang_fe` | Tương tự | `TypeScript`, `JavaScript`, `Dart`, `Swift`, `Kotlin` |
| `framework_be` | Tương tự | `Flask`, `FastAPI`, `Gin`, `Spring Boot`, `NestJS`, `Laravel`, `Rails` |
| `framework_fe` | Tương tự | `Next.js`, `React`, `Vue`, `Nuxt`, `Angular`, `Flutter` |
| `arch` | Mục "Architecture" hoặc layer description | `layered`, `MVC`, `hexagonal`, `microservices`, `CQRS` |
| `async_tech` | Mục "Worker" / "Queue" | `Celery`, `BullMQ`, `Sidekiq`, `Kafka`, `none` |
| `main_flow` | Mục "Flow chính" | `A → B → C` |
| `api_format` | Mục "API Contract" | `{ success, data, error, meta }` |
| `custom_rules` | Mục "Nguyên tắc bắt buộc" / "Hard Rules" | array of strings |

Nếu không tìm thấy → để trống, dùng universal rules.

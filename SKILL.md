...
Từ CLAUDE.md / Agents.md của các submodule đã load, extract:

| Biến | Nguồn | Ví dụ |
|------|-------|-------|
| `lang_be` | Mục Tech Stack hoặc Coding Conventions | `Python`, `Go`, `TypeScript/Node`, `Java`, `PHP`, `Ruby` |
| `lang_fe` | Mục Tech Stack hoặc Coding Conventions | `TypeScript`, `JavaScript`, `Dart`, `Swift`, `Kotlin` |
| `framework_be` | Tương tự | `Flask`, `FastAPI`, `Gin`, `Echo`, `Spring Boot`, `Laravel`, `Rails`, `Express`, `NestJS` |
| `framework_fe` | Tương tự | `Next.js`, `React`, `Vue`, `Nuxt`, `Angular`, `Flutter` |
| `arch_pattern` | Mục Architecture hoặc Coding Conventions | `layered`, `MVC`, `hexagonal`, `microservices`, `CQRS` |
| `async_tech` | Mục Worker / Async | `Celery`, `BullMQ`, `Sidekiq`, `Kafka`, `RQ`, `none` |
| `has_fe` | Phát hiện từ submodule | `true` / `false` |
| `has_be` | Phát hiện từ submodule | `true` / `false` |

...
Ba sub-command, mỗi cái thực sự làm việc:

| Lệnh | Kết quả |
|------|---------|
| `/bs-claude-toolkit plan [scope] task` | Claude chạy research scripts, rồi **tạo** `sprint-N-slug.md` với context, phân tích, và các bước implement |
| `/bs-claude-toolkit review [scope]` | Claude đọc `git diff`, rồi **apply** checklist theo stack vào code thực tế và output findings |
| `/bs-claude-toolkit [scope]` | Chỉ orientation — hiển thị stack, số sprint tiếp theo, và workflow |
...
Mỗi lần chạy skill cũng:

- **Load context project** — đọc config và cấu trúc submodule. Các lần sau dùng stack profile đã cache thay vì đọc lại file (~90% ít token hơn).
- **Tự detect tech stack** — quét `package.json`, `requirements.txt`, `go.mod`, v.v. Ghi kết quả vào `.bs-toolkit.json` để lần sau dùng cache.
- **Tính số sprint tiếp theo** — scan `*/docs/plan/` để tìm sprint mới nhất.
...
```
# Plan — Claude research rồi tạo sprint plan doc
/bs-claude-toolkit plan fix video retry không trigger
/bs-claude-toolkit plan be thêm pagination cho orders API
/bs-claude-toolkit plan fe refactor luồng upload asset

# Review — Claude đọc git diff và apply checklist
/bs-claude-toolkit review
/bs-claude-toolkit review be          ← chỉ review backend

# Brief — chỉ orientation, không tạo file
/bs-claude-toolkit                    ← hiển thị stack + sprint tiếp theo + workflow
...
File skill chính ở root repo. Claude Code tự load khi clone vào `~/.claude/skills/`. Chứa toàn bộ logic 6 phase: load config, detect stack, sprint intelligence, phân loại task, thực thi theo mode (`plan` / `review` / `brief`), và proactive warnings.

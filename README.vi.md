...
Skill Claude Code chạy quy trình sprint 3 bước có cấu trúc — từ planning (kèm test scaffold) qua implementation đến review cuối.
...
|------|---------|---------|
| `/bs-claude-toolkit plan [scope] <task>` | Claude | `sprint-N-slug.md` + test scaffold — context, phân tích, impact, risk, test cases |
| `/bs-claude-toolkit test [scope]` | Claude | Contract test scaffold + E2E scaffold + test plan doc *(standalone — tùy chọn)* |
...
### `/test` — Tạo integration test *(standalone)*

> **Trong main workflow, test scaffold được tạo tự động ở cuối `/plan`.** Dùng `/test` như lệnh standalone khi cần tái tạo scaffold sau khi code đã có (ví dụ scope thay đổi, endpoint thay đổi sau implement).
...
**Ví dụ** — tái tạo scaffold standalone sau khi scope thay đổi:
...
# Tái tạo tests standalone (tùy chọn — plan đã tạo scaffold rồi)
/bs-claude-toolkit test
/bs-claude-toolkit test be          ← chỉ contract tests cho BE
/bs-claude-toolkit test fe          ← chỉ E2E tests cho FE

# Review sau khi Codex implement, chạy tests, và tạo docs
